...
def get_target(label_file,Tr):
    target = np.zeros([50, 8], dtype = np.float32)
    ...
    for j in range(num_obj):
        ...
        # if the class is the one we need
        if obj_class in class_list:
            #get the translate vector and rotate matrix
            t_lidar, box3d_corner = box3d_cam_to_velo(obj[8:], Tr)   # get target 3D object location x,y
            # center x, y of the object box
            location_x = t_lidar[0][0]
            location_y = t_lidar[0][1]
            # verify if the obj is in the roi
            if  (location_x > 0) & (location_x < 40) & (location_y > -40)  & (location_y < 40) :
                # nomalize y, x
                target[index][2] = location_x / 40             # make sure target inside the covering area (0,1)
                target[index][1] = (location_y + 40) / 80             ## we should put this in [0,1] ,so divide max_size  80 m
                # label width height
                ...
                rotate_camY = obj[14].strip()
                rotate_z = - math.pi / 2 + float(rotate_camY)
                # get target Observation angle of object, ranging [-pi..pi]
                target[index][5]=math.sin(float(rotate_z))    #complex YOLO   Im
                target[index][6]=math.cos(float(rotate_z))    #complex YOLO   Re
